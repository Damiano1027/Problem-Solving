...
public class Main {
    ...
    static void solve() {
        result += step1();
        step2(matrix, fishes);
        result += step3(matrix, shark, 0);

        //print(copiedMatrix(matrix));
    }
    ...
    static void step2(Animal[][] matrix, List<Fish> fishes) {
        ...
        for (Fish fish: fishes) {
            step2AboutOneFish(matrix, fish);
        }
    }
    static void step2AboutOneFish(Animal[][] matrix, Fish fish) {
        for (int count = 0; count < 8; count++) {
            int nextRow = fish.row + dRow[fish.direction];
            int nextCol = fish.col + dCol[fish.direction];

            // 이동할 수 없는 방향인 경우
            if (!isInMatrix(nextRow, nextCol) || isShark(matrix[nextRow][nextCol])) {
                fish.direction++;
                if (fish.direction > 8) {
                    fish.direction = 1;
                }
            ...
            if (matrix[nextRow][nextCol] == null) {
                ...
                fish.row = nextRow;
                fish.col = nextCol;
            } else if (isFish(matrix[nextRow][nextCol])) {
                swapFish(fish, (Fish) matrix[nextRow][nextCol], matrix);
            }
    ...
    static boolean isShark(Animal animal) {
        return animal != null && animal.getClass().equals(Shark.class);
    }

    static boolean isFish(Animal animal) {
        return animal != null && animal.getClass().equals(Fish.class);
    }

    static void swapFish(Fish fish1, Fish fish2, Animal[][] matrix) {
     ...
     */
    static int step3(Animal[][] rMatrix, Shark rShark, int eatenFishesNumberSum) {
        ...
        for (Location fishLocation: fishLocationsInDirection) {
            ...
            Shark copiedShark = copiedShark(copiedMatrix);
            List<Fish> copiedFishes = copiedFishes(copiedMatrix);
            ...
            max = Math.max(max, step3(copiedMatrix, copiedShark, eatenFishesNumberSum + copiedFish.number));
        }
    ...
    static Shark copiedShark(Animal[][] matrix) {
        Shark shark = null;
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                if (isShark(matrix[row][col])) {
                    shark = (Shark) matrix[row][col];
                    break;
                }
            }
        }
    ...
    static List<Fish> copiedFishes(Animal[][] matrix) {
        List<Fish> fishes = new LinkedList<>();

        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                if (isFish(matrix[row][col])) {
                    fishes.add((Fish) matrix[row][col]);
                }
            }
        }
