...
public class Main {
    ...
    static void step(List<Tile> addedTiles) {
        ...
        for (int col = 9; col >= 6; col--) {
            ...
            List<Tile> copiedCurrentTiles = new ArrayList<>(currentTiles);
            currentTiles = new HashSet<>();

            for (Tile copiedCurrentTile: copiedCurrentTiles) {
                if (isBlueArea(copiedCurrentTile) && copiedCurrentTile.col < col) {
            ...
            for (int row = 0; row <= 3; row++) {
                if (currentTiles.contains(new Tile(row, col))) {
                    for (int tempRow = 0; tempRow <= 3; tempRow++) {
                        currentTiles.remove(new Tile(tempRow, 9));
                    }
        ...
        for (int row = 9; row >= 6; row--) {
            ...
            for (Tile copiedCurrentTile: copiedCurrentTiles) {
                if (isGreenArea(copiedCurrentTile) && copiedCurrentTile.row < row) {
            ...
            for (int col = 0; col <= 3; col++) {
                if (currentTiles.contains(new Tile(row, col))) {
                    for (int tempCol = 0; tempCol <= 3; tempCol++) {
                        currentTiles.remove(new Tile(9, tempCol));
                    }
