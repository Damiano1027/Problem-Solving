...
public class Main {
    ...
    static void dfs(int[][] matrix, int count) {
        result = Math.max(result, getBlockMaxValue(matrix));
        ...
        for (int direction = 0; direction < 4; direction++) {
            ...
            if (equals(matrix, movedMatrix)) {
                continue;
            }

            dfs(movedMatrix, count + 1);
        }
    }
    ...
    static boolean equals(int[][] matrix1, int[][] matrix2) {
        for (int row = 0; row < N; row++) {
